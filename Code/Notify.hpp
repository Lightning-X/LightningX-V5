...
#define NOTIFY_FADE_IN_OUT_TIME			350			// Fade in and out duration
...
enum ImGuiToastType_
{
	...
	ImGuiToastType_Info,
	ImGuiToastType_Protections,
	ImGuiToastType_PlayerJoined,
	ImGuiToastType_PlayerLeft,
	ImGuiToastType_FriendLeft,
	ImGuiToastType_FriendJoined,
	...
	NOTIFY_INLINE auto get_default_title() -> const char*
	{
		if (!strlen(this->title))
		{
			...
			case ImGuiToastType_Info:
				return "Info";
			case ImGuiToastType_Protections:
				return "Protections";
			case ImGuiToastType_PlayerJoined:
				return "PlayerList";
			case ImGuiToastType_PlayerLeft:
				return "PlayerList";
			case ImGuiToastType_FriendLeft:
				return "FriendList";
			case ImGuiToastType_FriendJoined:
				return "FriendList";
			default:
				return NULL;
			}
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			// Prevent toast from being looped if text is same as previous
			if (i > 0 && strcmp(notifications[i - 1].get_content(), content) == 0) {
				RemoveNotification(i);
				continue;
			}

			// only display 13 notifications at one time
			if (notifications.size() > 13) {
				RemoveNotification(0); // 0 because its the very first (old)
				continue;
			}
			...
			std::string window_name = std::format("##TOAST{}", i);
			...
			{
				PushTextWrapPos(vp_size.x / 4.f); // We want to support multi-line text, this will wrap the text after 1/4 of the screen width
				...
				if (!NOTIFY_NULL_OR_EMPTY(icon))
				{
					...
					PushFont(g_renderer->m_bigfontawesome);
	...
	NOTIFY_INLINE VOID MergeIconsWithLatestFont(float font_size, bool FontDataOwnedByAtlas = false)
	{
		...
		GetIO().Fonts->AddFontFromFileTTF(R"(C:\LightningFiles\Fonts\fa-solid-900.ttf)", font_size, &icons_config, icons_ranges);
	}
}
template <typename ...arguments>
inline void MainNotification(ImGuiToastType_ type, int timer, const char* title, const char* text, arguments... args) {
	ImGuiToast toast(type, timer);
	toast.set_title(title);
	toast.set_content(text, args...);
	ImGui::InsertNotification(toast);
}
inline void above_map(std::string text)
{
	base::HUD::SET_TEXT_OUTLINE();
	base::HUD::BEGIN_TEXT_COMMAND_THEFEED_POST("STRING");
	base::HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(text.data());
	base::HUD::END_TEXT_COMMAND_THEFEED_POST_MESSAGETEXT_WITH_CREW_TAG_AND_ADDITIONAL_ICON("CHAR_LESTER_DEATHWISH", "CHAR_LESTER_DEATHWISH", true, 5, brandingName, "~c~Notification", 1.f, "___LX", 5, 0);
	base::HUD::END_TEXT_COMMAND_THEFEED_POST_TICKER(FALSE, FALSE);
}
inline void display_help_text(std::string text)
{
	base::HUD::BEGIN_TEXT_COMMAND_DISPLAY_HELP("STRING");
	base::HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(text.data());
	base::HUD::END_TEXT_COMMAND_DISPLAY_HELP(0, 0, 1, -1);
}
#endif
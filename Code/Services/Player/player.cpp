...
#include "../Friend/friends_service.hpp"
...
namespace base
{
	...
	{
		return get_net_game_player() == nullptr ? "" : get_net_data()->m_name;
	}

	rage::rlGamerInfo* player::get_net_data() const
	{
		return get_net_game_player() == nullptr ? nullptr : &get_player_info()->m_net_player_data;
	}
	...
	{
		if (get_net_game_player() != nullptr && get_net_game_player()->m_player_info != nullptr)
			return get_net_game_player()->m_player_info;
		return nullptr;
	}
	...
	netAddress player::get_ip_address()
	{
		...
		if (auto session_player = get_session_player())
			if (auto peer = g_pointers->m_getConnectionPeer(gta_util::get_network()->m_game_session_ptr->m_net_connection_mgr,
	...
	{
		return get_net_game_player() == nullptr ? -1 : get_net_game_player()->m_player_id;
	}

	bool player::is_host() const
	{
		return get_net_game_player() == nullptr ? false : get_net_game_player()->IsNetworkHost();
	}
	...
	{
		return get_net_game_player() == nullptr ? false : get_net_game_player()->IsConnected();
	}

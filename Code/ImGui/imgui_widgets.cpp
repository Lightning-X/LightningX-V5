...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(IM_COL32(150, 150, 150, 255)), style.FrameRounding);
	...
	if (mixed_value)
	{
		...
		window->DrawList->AddRect(check_bb.Min + pad, check_bb.Max - pad, check_col, style.FrameRounding);
	}
	else if (*v)
	{
		//const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
		//RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
		window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(IM_COL32(0, 183, 254, 255)), style.FrameRounding);
	}
